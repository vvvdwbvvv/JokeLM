...
def direct_answer(query):
    ...
    response = client.get_ollama_message(
        messages=prompt,
        model='qwen3:4b'  # You can use a different model if needed
    )
    return response
...
if __name__ == "__main__":
    ...
    for i, topic in enumerate(topics):
        ...
        generated_joke = direct_answer(query)
    ...
    output_filename = os.path.join(output_dir, 'generated_and_evaluated_jokes_origin.json')
